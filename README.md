...
**已驗證可成功建置**（2026-09-14，commit b3fdb6b，含 Crashlytics 根因修正批次）：透過 Firebase Crashlytics 找到 App 重開崩潰的真正原因——R8 誤刪 flutter_local_notifications 內部 Gson TypeToken 所需的泛型型別資訊（跟先前 Firebase/WorkManager 是同一類問題），加上正確的 ProGuard 規則解決，這才是「關閉後無法重開」跟「提醒不會跳出來」的真正根因。

其他修正：
- 電池優化排除按鈕只能點一次：第二次改成直接開啟系統設定頁
- 語音測試頁「還原預設」按鈕文字看不到：移除寫死的白色文字
- AppBar 標題直屏被擠壓：多餘圖示整合進溢出選單
- 整體配色改為更柔和的鼠尾草綠/藍綠色系（依色彩心理學研究：冷色調助於放鬆專注、利於長期記憶）
- 語音測試頁加入「還原預設語音」按鈕
- 學習統計畫面四份教材都加上各自的說明文字
- 待確認：AdMob 廣告顯示（新廣告單元，24-48小時內未顯示屬正常）
- 待規劃：多語言翻譯內容（架構已支援，日/韓/越翻譯尚未填入）、中國大陸市場（需要完全不同的技術棧，Firebase/AdMob/Google Play 在當地都無法使用，暫緩評估）
